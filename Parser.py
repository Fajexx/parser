...
reservadas = ['DOCTYPE', 
              ...
              'OPEN_IMAGEDATA','OPEN_VIDEO_DATA',"CLOSE_VIDEO_DATA","CLOSE_IMAGE_DATA"]
...
def t_IMAGE_DATA(t):
      r'"((https|https|ftp|ftps)?://[^\s<>"]+|www)\.[^\s<>"]+\.(png|jpg|jpeg|gif|bmp|tiff|pdf|eps|svg)">'
      return t
...
def t_OPEN_LINK (t):
    ...
    global reservadas,url
    url = t.value[17: -1]
    html.write(f'<a href={url}>')
    reservadas.append(f"<a link href={t.value[17: -1]}>")
    return t
...
def p_adr (p):
    ''' adr : OPEN_ADDRESS genad CLOSE_ADDRESS
        | OPEN_ADDRESS imprimir CLOSE_ADDRESS '''
...
def p_j (p):
    ''' j : OPEN_INFO info CLOSE_INFO video r
        ...
        | OPEN_IMAGE_OBJECT OPEN_IMAGEDATA IMAGE_DATA CLOSE_MEDBRAC CLOSE_IMAGE_DATA CLOSE_IMAGE_OBJECT'''
...
def p_image (p):
    ''' image : OPEN_IMAGE_OBJECT info OPEN_IMAGEDATA CLOSE_MEDBRAC IMAGE_DATA CLOSE_IMAGE_OBJECT
            | OPEN_IMAGE_OBJECT OPEN_IMAGEDATA IMAGE_DATA CLOSE_MEDBRAC CLOSE_IMAGE_DATA CLOSE_IMAGE_OBJECT '''
...
if rta.upper() == "A":
    ...
    if 1 <= num_archivo <= len(archivos):
        ...
        html.write('''
          ...
          <body>
        ''')  
#nombre_html = nombre_html(nombre_archivo)
